...
export default function EditStaffPage() {
  ...
  useEffect(() => {
    const fetchStaff = async () => {
      try {
        const { data } = await axios.get(`https://smart-rental-management.onrender.com/api/admin/profile/${id}`);
        setStaff(data);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.patch(`https://smart-rental-management.onrender.com/api/admin/profile/${id}`, staff);
      ...
      <form onSubmit={handleSubmit} className="card p-4 shadow-sm">
        ...
        <div className="form-check mb-3">
          <input
            ...
            onChange={() => setStaff({ ...staff, suspended: !staff.suspended })}
          />
